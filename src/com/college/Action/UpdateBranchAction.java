...
public class UpdateBranchAction extends HttpServlet {
	...
	try {
	  ...
	  try{
		  ...
		  int cid=Integer.parseInt(request.getParameter("bid"));
		   ...
		   flag= new CourseMgrDelegate().updateBranch(cb);
		 
		  System.out.println("in Action class vcb..........."+flag);
		  
		  
		  if(flag){
			  request.setAttribute("status", " Branch Updatation  is Success");
			  path="./ViewBranchAction?page=./ViewAllBranch.jsp";
			 }
		  else {
			  request.setAttribute("status", "Updation of Branch is Failed ");
			  path="./ViewBranchAction?page=./ViewAllBranch.jsp";
			  }
