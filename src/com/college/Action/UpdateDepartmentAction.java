	...
	try {
	  ...
	  try{
		 ...
		 // int cid=Integer.parseInt(request.getParameter("did"));
		   ...
		   flag= new CourseMgrDelegate().updateDepartment(cb);
		 
		  System.out.println("in Action class vcb..........."+flag);
		  
		  
		  if(flag){
			  request.setAttribute("status", " Department Updatation  is Success");
			  path="./ViewDepartmentAction?page=./ViewAllDepartment.jsp";
			 }
		  else {
			  request.setAttribute("status", "Updation of Department is Failed ");
			  path="./ViewDepartmentAction?page=./ViewAllDepartment.jsp";
			  }
