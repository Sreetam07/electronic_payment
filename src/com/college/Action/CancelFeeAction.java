...
public class CancelFeeAction extends HttpServlet {
	...
	try {
	  ...
	  try{ 
		  ...
		  int cid=Integer.parseInt(request.getParameter("fid"));
	         ...
	         flag= new CourseMgrDelegate().deleteFee(cid);		
		
		  
		System.out.println("in Action class flag..........."+flag);
		  
		 if(flag){
			  request.setAttribute("status","Fees Canceled Successfully");
			  path="./ViewFeeAction?page=./ViewAllFee.jsp";
			 }
		  else {
			  request.setAttribute("status","Cancelation of Fees is Failed");
			  path="./ViewFeeAction?page=./ViewAllFee.jsp";
		}
		   }
	  catch (Exception e) {
